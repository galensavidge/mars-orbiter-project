...
[time, sigma, w, sigma_ref, w_ref, mode] = ...
    rigid_body_integrator(mrp_BN_0, w_BN_B_0, I, @gmo_pointing_reference, t0, dt, t1);

% Outputs
sigma15 = sigma(:,15/dt+1);
sigma_file = fopen("Task 10/sigma15.txt",'w');
fprintf(sigma_file, '%.12f %.12f %.12f', sigma15);
sigma100 = sigma(:,100/dt+1);
sigma_file = fopen("Task 10/sigma100.txt",'w');
fprintf(sigma_file, '%.12f %.12f %.12f', sigma100);
sigma200 = sigma(:,200/dt+1);
sigma_file = fopen("Task 10/sigma200.txt",'w');
fprintf(sigma_file, '%.12f %.12f %.12f', sigma200);
sigma400 = sigma(:,400/dt+1);
sigma_file = fopen("Task 10/sigma400.txt",'w');
...
saveas(gcf, 'Task 10/combined_plot.png')

function[Rn, w, mode] = gmo_pointing_reference(t)
    Rn = calc_Rc_frame(t);
    w = calc_w_Rc_N(t);
    mode = 1;
end